...
export class ExtractionZone {
  ...
  private readonly requiredHoldTime = 5.0
  private radius = 5.0
  ...
  activate(position: THREE.Vector3): void {
    ...
    useGameStore.getState().showCallout(t("callout.extraction.deployed", undefined, locale), 3000)
  }
  ...
  update(delta: number, vehicle: Vehicle): void {
    ...
    if (dist < this.radius) {
      ...
      if (remaining !== this.lastAnnouncedSecond && remaining > 0) {
        ...
        const locale = useGameStore.getState().locale
        useGameStore.getState().showCallout(t("callout.extraction.extracting", { seconds: remaining }, locale), 900)
      }
      ...
      if (this.holdTimer > 0) {
        const locale = useGameStore.getState().locale
        useGameStore.getState().showCallout(t("callout.extraction.interrupted", undefined, locale), 1500)
      }
      this.holdTimer = 0
      this.lastAnnouncedSecond = -1
  ...
  private triggerExtraction(): void {
    ...
    this.group.visible = false
    const locale = useGameStore.getState().locale
    useGameStore.getState().showCallout(t("callout.extraction.success", undefined, locale), 3000)
